...
| File | Description |
|------|-------------|
| `scraper/index.js` | Main scraper - full workflow: validate company → scrape ANOFM → transform → upsert → generate docs/jobs.md |
...
| `scraper/job-validator.js` | Shared validation primitives - exports validateByHead(url), validateByContent(url, opts), validateByBrowser(url, opts), DEFAULT_EXPIRED_KEYWORDS. Used by `validate-jobs.js` and `tests/validate-gusturi-divine-jobs.js`. |
...
| `scraper/config/scraper.json` | Scraper-specific config - ANOFM API base URL and list path |
...
| `tests/validate-gusturi-divine-jobs.js` | **GUSTURI DIVINE-specific validator (used by CI).** HEAD checks via `validateByHead`. Called nightly by `automation-testing.yml`. Supports `--dry-run` and `--delete`. |
| `tests/unit/index.test.js` | Unit tests for index.js - mapToJobModel, transformJobsForSOLR, searchANOFM |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real ANOFM data, ANAF, and Peviitor API |
...
| `ai/ROBOTS.md` | robots.txt analysis and scraping policy for ANOFM |
...
- Full workflow: validate company (ANAF+CUIScan+CUIFirma+Peviitor) → scrape ANOFM → transform → upsert → generate docs/jobs.md
