...
This scraper extracts job listings from ANOFM (Agentia Nationala pentru Ocuparea Fortei de Munca) for GUSTURI DIVINE S.R.L. and imports them to peviitor.ro.

Data source: `https://mediere.anofm.ro/api/entity/vw_public_job_posting` (queried by CIF `47473595`)
...
1. **Start with brand** - We know the brand ("GUSTURI DIVINE")
...
8. **Scrape new jobs** - Extract jobs from ANOFM API (by CIF)
...
> **Important**: The scraper only upserts ANOFM jobs for this CIF. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from ANOFM API (by CIF)
...
| `scraper/config/scraper.json` | ANOFM API config (base URL, list path, default location) |
...
| `tests/validate-gusturi-divine-jobs.js` | CI fast validator (HEAD only); thin CLI over `scraper/job-validator.js` + `scraper/api.js` |
...
| `tests/e2e/scraper.test.js` | End-to-end tests with real ANOFM data |
